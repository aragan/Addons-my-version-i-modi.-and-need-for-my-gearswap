...
function calculate_target_type_rating(mob)
    ...
    -- Assign the highest priority to these names
    if string.match(mob.name, "Operative") or string.match(mob.name, "Shinobi") or string.match(mob.name, "Shadowstalker") or string.match(mob.name, "Spy") or string.match(mob.name, "Ninja") or string.match(mob.name, "Hitman") or string.match(mob.name, "Agon Halo") or string.match(mob.name, "Azi Dahaka") or string.match(mob.name, "Naga Raja") or string.match(mob.name, "Quetzalcoatl") or string.match(mob.name, "Mireu") then
        windower.add_to_chat(158, 'Priority Target Found: ' .. mob.name)
        return 1
    end
    
    -- for _, mob in pairs(mobs) do
    --     if string.match(mob.name, "Operative") or string.match(mob.name, "Shinobi") or string.match(mob.name, "Shadowstalker") or string.match(mob.name, "Spy") then
    --         return mob -- اختياره فورًا بدون حسابات أخرى
    --     end
    -- end
    ...
    if string.match(mob.name, "Commander") or string.match(mob.name, "Leader") or string.match(mob.name, "Lamia") or string.match(mob.name, "Agon") then
        return 3
    end
...
function calculate_target_rating(mob, player_mob, party, not_aggro_okay)
    ...
    -- ✅ الأولوية: NMات ثم Statues ثم باقي الأنواع ثم Aggro fallback
    if type_rating then
        return (type_rating * 10) + distance_rating
    elseif mob.status == 1 then
        -- إذا العدو يهاجم فعليًا (aggro) ولم يوجد NM أو Statue
        return 100  -- قيمة كافية ليكون التالي في الترتيب
    else
        return 999  -- ضعيف تمامًا (لا يُستهدف)
    end
