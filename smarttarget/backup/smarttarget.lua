...
windower.register_event('prerender', function()
    if not mob or not player_mob then
        return
    end

    -- حساب الاتجاه بين اللاعب والعدو
    local angle = math.atan2(mob.y - player_mob.y, mob.x - player_mob.x)

    -- جعل اللاعب يتحرك باتجاه العدو
