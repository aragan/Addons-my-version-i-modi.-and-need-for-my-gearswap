...
_addon.version = '4.4'
_addon.author = 'PBW , Aragan' ---- modi. by Author: Aragan add command //autoitem toggle to use 1 bind  
...
dot = true
job_registry = T{}
panacea_buffs = S{13,134,136,138,139,140,141,144,145,146,147,148,149,167,564}	-- Slow, Frost, Drown, STR Down, VIT Down,INT Down, AGI Down, MND Down, Dia, Max HP Down, Max MP Down, Accuracy Down, Attack Down, Evasion Down, Defense Down, Magic Def. Down, Magic Evasion Down,
    ...
    for buff_id,_ in pairs (active_buffs) do
		if doom_buffs:contains(buff_id) and (os.clock()-attempt) > 1.5 and (player.main_job ~= 'WHM' or (player.main_job == 'WHM' and windower.ffxi.get_spell_recasts()[20] > 0)) then
            if haveBuff(buff_id) and haveMeds(4154) then
				windower.add_to_chat(207,"[AutoItem] DOOMED - Using Holy Water.")
		...
		elseif active and remedy_buffs:contains(buff_id) and (os.clock()-attempt) > 2.5 then
			if haveBuff(buff_id) and haveMeds(4155) then
				windower.add_to_chat(207,"[AutoItem] Using Remedy.")
		...
		elseif active and ((panacea and panacea_buffs:contains(buff_id)) or (dot and dot_buffs:contains(buff_id))) and (os.clock()-attempt) > 3.5 then
			if haveBuff(buff_id) and haveMeds(4149) then
				windower.add_to_chat(207,"[AutoItem] Using Panacea.")
		...
		elseif active and holywater_buffs:contains(buff_id) and player.main_job ~= 'WHM' and (os.clock()-attempt) > 3.5 then
            if haveBuff(buff_id) and haveMeds(4154) then
				windower.add_to_chat(207,"[AutoItem] Using Holy Water.")
				windower.send_command('input /item "Holy Water" <me>')
				attempt = os.clock()
            else
				active_buffs:remove(buff_id)
...
function haveMeds(med_id)
	...
	windower.add_to_chat(207, '[AutoItem] <<NO>> -' .. res.items[med_id].en .. '- Found!')
	return false
end
...
function handle_addon(...)
    ...
    if args[1] ~= nil then
        ...
        if comm == 'on' then
            active = true
			windower.add_to_chat(207,"[AutoItem] ON")
        elseif comm == 'off' then
			active = false
            windower.add_to_chat(207,"[AutoItem] OFF")
		...
		elseif comm == 'pana' then
			if args[2] and args[2]:lower() == 'on' then
				panacea = true
				windower.add_to_chat(207,"[AutoItem] Panacea ON")
			elseif args[2] and args[2]:lower() == 'off' then
				panacea = false
				windower.add_to_chat(207,"[AutoItem] Panacea OFF")
			else
				windower.add_to_chat(207,"[AutoItem] No parameter specified.")
			end
		elseif comm == 'dot' then
			if args[2] and args[2]:lower() == 'on' then
				dot = true
				windower.add_to_chat(207,"[AutoItem] DoT ON")
			elseif args[2] and args[2]:lower() == 'off' then
				dot = false
				windower.add_to_chat(207,"[AutoItem] DoT OFF")
			else
				windower.add_to_chat(207,"[AutoItem] No parameter specified.")
			end
...
windower.register_event('load', function()
	windower.add_to_chat(207,'[AutoItem] Welcome to AutoItem!')
end)

function handle_zone_change(new_id, old_id)
	if panacea then
		windower.add_to_chat(207,'[AutoItem] Disabling Auto-Panacea.')
		panacea = false
	end
end
...
-- code for panaca on off toggle -- autoitem pana toggle add ny aragan for archive
...
--         panacea = not panacea
--         if panacea then
--             windower.add_to_chat(207,"[AutoItem] Panacea TOGGLED ON")
--         else
--             windower.add_to_chat(207,"[AutoItem] Panacea TOGGLED OFF")
--         end
--     else
--         windower.add_to_chat(207,"[AutoItem] Invalid parameter: " .. subcmd)
--     end
-- else
--     windower.add_to_chat(207,"[AutoItem] No parameter specified.")
-- end