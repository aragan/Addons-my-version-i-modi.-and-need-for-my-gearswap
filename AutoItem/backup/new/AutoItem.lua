...
panacea = true
job_registry = T{}
panacea_buffs = S{131,13,129,133,134,136,138,139,140,141,144,145,146,147,148,149,167,564}	-- Slow, Frost, Drown, STR Down, VIT Down,INT Down, AGI Down, MND Down, Dia, Max HP Down, Max MP Down, Accuracy Down, Attack Down, Evasion Down, Defense Down, Magic Def. Down, Magic Evasion Down,
dot_buffs = S{128,129,130,131,132,133}
remedy_buffs = S{3,4,8}	-- Paralysis, Disease,poison
holywater_buffs = S{9,15}	-- Curse,doom
...
function handle_lose_buff(buff_id)
	if buff_id and allbuffs:contains(buff_id) then
		...
		if panacea and active and (panacea_buffs:contains(buff_id) or dot_buffs:contains(buff_id)) then
			windower.add_to_chat(121,'[AutoItem] Debuff removed: ' .. res.buffs[buff_id].en .. ' - '..'['..buff_id..']')
...
function handle_addon(...)
    ...
    if args[1] ~= nil then
        ...
        if comm == 'on' then
            active = true
			windower.add_to_chat(121,"[AutoItem] ON")
        elseif comm == 'off' then
			active = false
            windower.add_to_chat(121,"[AutoItem] OFF")
		elseif comm == 'pana' then
			if args[2] and args[2]:lower() == 'on' then
				panacea = true
				windower.add_to_chat(121,"[AutoItem] Panacea ON")
			elseif args[2] and args[2]:lower() == 'off' then
				panacea = false
				windower.add_to_chat(121,"[AutoItem] Panacea OFF")
			else
				windower.add_to_chat(121,"[AutoItem] No parameter specified.")
			end
...
windower.register_event('load', function()
	windower.add_to_chat(121,'[AutoItem] Welcome to AutoItem!')
end)
